...
const initialPreReadingProfile = {
  mood: '',
  readingStyle: '',
  seekerName: '',
  seekerBirthDate: '',
...
const LAST_PROFILE_KEY = 'spirit-guide:last-pre-reading-profile'
...
export function GestureSessionProvider({ children }) {
  ...
  const [preReadingProfile, setPreReadingProfile] = useState(initialPreReadingProfile)
  const [lastPreReadingProfile, setLastPreReadingProfile] = useState(
    initialPreReadingProfile,
  )
  ...
  useEffect(() => {
    try {
      ...
      if (parsed?.mood && parsed?.readingStyle) {
        setLastPreReadingProfile({ ...initialPreReadingProfile, ...parsed })
      }
    } catch {
  ...
  useEffect(() => {
    if (!preReadingProfile.mood || !preReadingProfile.readingStyle) return
    setLastPreReadingProfile(preReadingProfile)
    try {
      window.localStorage.setItem(LAST_PROFILE_KEY, JSON.stringify(preReadingProfile))
    } catch {
  ...
  const applyLastPreReadingProfile = useCallback(() => {
    if (!lastPreReadingProfile?.mood || !lastPreReadingProfile?.readingStyle) return false
    setPreReadingProfile((prev) => ({
      ...initialPreReadingProfile,
      ...lastPreReadingProfile,
      seekerName:
        (prev.seekerName || '').trim() || lastPreReadingProfile.seekerName || '',
      seekerBirthDate:
        (prev.seekerBirthDate || '').trim() || lastPreReadingProfile.seekerBirthDate || '',
      seekerMbti: (prev.seekerMbti || '').trim() || lastPreReadingProfile.seekerMbti || '',
    }))
    return true
  }, [lastPreReadingProfile])
  ...
  const value = useMemo(
    () => ({
      ...
      setUserQuestion,
      preReadingProfile,
      setPreReadingProfile,
      lastPreReadingProfile,
      applyLastPreReadingProfile,
      dispatch,
      beginRitual,
      setSpread,
      resetRitual,
