...
import { RitualPreparationModal } from './RitualPreparationModal'
import { SeekerWelcomeModal } from './SeekerWelcomeModal'
...
export function PhaseHud() {
  ...
  /** After greeting + seeker fields, show mood/style modal (reset each PRE_START entry). */
  const [sageWelcomeDone, setSageWelcomeDone] = useState(false)
  ...
  const hasLastProfile = Boolean(
    lastPreReadingProfile?.mood && lastPreReadingProfile?.readingStyle,
  )

  useEffect(() => {
    if (phase !== 'PRE_START') return
    setPrepCompleted(false)
    setSageWelcomeDone(false)
    setShowPrep(true)
  }, [phase])
  ...
  return (
    <div className="pointer-events-auto absolute right-6 top-24 z-20 w-[min(92vw,280px)] rounded-xl border border-white/10 bg-black/45 p-4 shadow-xl backdrop-blur-md md:top-28">
        ...
        <>
          {!prepCompleted ? (
            <button
              ...
              className="mt-4 w-full rounded-lg border border-violet-300/35 bg-violet-500/15 py-2.5 font-sans text-xs text-violet-100 transition hover:bg-violet-500/25"
            >
              {t('ritualPrep.title')}
            </button>
          ) : (
      ...
      <RitualPreparationModal
        open={phase === 'PRE_START' && showPrep && sageWelcomeDone}
        profile={preReadingProfile}
        hasLastProfile={hasLastProfile}
        onMoodChange={(mood) =>
        ...
        onReuseLast={() => {
          const ok = applyLastPreReadingProfile()
          if (!ok) return
          setSageWelcomeDone(true)
          setShowPrep(false)
          setPrepCompleted(true)
        }}
      ...
      <SeekerWelcomeModal
        ...
        profile={preReadingProfile}
        onConfirm={(fields) => {
          setPreReadingProfile((prev) => ({ ...prev, ...fields }))
          setSageWelcomeDone(true)
        }}
        onClose={() => setShowPrep(false)}
      />
